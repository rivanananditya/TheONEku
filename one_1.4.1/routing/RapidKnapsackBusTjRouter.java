 ...
 * RAPID router
 */
public class RapidKnapsackBusTjRouter extends ActiveRouterForKnapsack {
     ...
     */
    public RapidKnapsackBusTjRouter(Settings s) {
        super(s);
...
//        System.out.println(host);
     ...
     */
    protected RapidKnapsackBusTjRouter(RapidKnapsackBusTjRouter r) {
        super(r);
...
//            }
    ...
    public int receiveMessage(Message m, DTNHost from) {
        ...
        if (stat == 0) {
            ...
            ((RapidKnapsackBusTjRouter) from.getRouter()).updateDelayTableEntry(m, host, delay, time);
        }
    ...
    private double getMarginalUtility(Message msg, DTNHost otherHost, DTNHost host) {
        final RapidKnapsackBusTjRouter otherRouter = (RapidKnapsackBusTjRouter) (otherHost.getRouter());
     ...
     * @return the current MU value
     */
    private double getMarginalUtility(Message msg, RapidKnapsackBusTjRouter router, DTNHost host) {
    ...
    private double computeRemainingTime(Message msg) {
        ...
        if (this.delayTable.getDelayEntryByMessageId(msg.getId()) != null) {
            ...
            for (Entry<DTNHost, Tuple<Double, Double>> entry : this.delayTable.getDelayEntryByMessageId(msg.getId()).getDelays()) {
                ...
                transferTime = ((RapidKnapsackBusTjRouter) host.getRouter()).computeTransferTime(msg, msg.getTo()); //MXm(i)	with m element of [0...k]
