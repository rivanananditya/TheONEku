...
public abstract class MovementModel {
    ...
    /**
     * MovementModel namespace (where world size and rng seed settings are
     * looked from ({@value})
     */
    ...
    /**
     * Checks that the minimum setting is not bigger than the maximum and that
     * both are positive
     *
    ...
    /**
     * Generates and returns a suitable waiting time at the end of a path. (i.e.
     * random variable whose value is between min and max of the
     * {@link #WAIT_TIME} setting).
     *
    ...
    /**
     * Returns a new path by this movement model or null if no new path could be
     * constructed at the moment (node should wait where it is). A new path
     * should not be requested before the destination of the previous path has
     * been reached.
     *
